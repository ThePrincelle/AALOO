...
for i in ${tp_diagrams[@]}; do
    ...
    do
        ...
        if [ $docker == 1 ]; then
            ...
            npx -p @mermaid-js/mermaid-cli mmdc -p ci/puppeteer-config.json -i $file -o output/$i/$filename/$filename.png -w 8000 -H 6000 -b white
        fi
        ...
        if [ $docker == 1 ]; then
            ...
            npx -p @mermaid-js/mermaid-cli mmdc -p ci/puppeteer-config.json -i $file -o output/$i/$filename/$filename.pdf
        fi
        ...
        if [ $docker == 1 ]; then
            ...
            npx -p @mermaid-js/mermaid-cli mmdc -p ci/puppeteer-config.json -i $file -o output/$i/$filename/$filename.svg
        fi
    done
done